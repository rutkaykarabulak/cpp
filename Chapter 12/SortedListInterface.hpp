 ...
 * ADT Sorted list interface.
 */

#ifndef SortedListInterface_hpp
#define SortedListInterface_hpp
...
public:
    ...
    @pre None.
    @post newEntry is in the list, and the list is sorted. @param newEntry The entry to insert into the sorted list. */
    ...
    @pre None.
    @post If the removal is successful, the first occurrence of the given entry is no longer in the sorted list, and the returned value is true. Otherwise, the sorted list is unchanged and the returned value is false.
    ...
    // in Listing 8-1 of Chapter 8 and are completely specified there. /** Sees whether this list is empty. */
