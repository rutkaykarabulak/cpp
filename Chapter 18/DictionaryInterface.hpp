...
public:
    ...
    @post If the insertion is successful, newItem is in its proper position within the dictionary.
    @param searchKey The search key associated with the item to be inserted. @param newItem The item to add to the dictionary.
    ...
    @post If the item whose search key equals searchKey existed in the dictionary, the item was removed.
    ...
    @post The given function’s action occurs once for each item in the dictionary and possibly alters the item.
